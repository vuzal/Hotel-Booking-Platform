    ...
    <div className="group bg-white rounded-[32px] border border-slate-100 overflow-hidden transition-all duration-500 hover:shadow-[0_20px_50px_rgba(0,0,0,0.05)] hover:-translate-y-1">
      <div className="flex flex-col md:flex-row">
        <div className="relative w-full md:w-56 h-48 md:h-auto overflow-hidden">
          <img src={booking.hotelImage || defaultImage} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" alt={booking.hotelName} />
        ...
        <div className="flex-1 p-6 md:p-8 flex flex-col justify-between">
          ...
          <div className="flex flex-wrap items-center gap-3 mt-8 pt-6 border-t border-slate-50">
            ...
            {booking.status === 'confirmed' && (
              <Button variant="outline" className="rounded-xl h-10 px-6 font-bold text-xs text-red-500 border-red-100 hover:bg-red-50" onClick={() => onCancel(booking.id)}>
                Ləğv et
              </Button>
            )}
...
export default function DashboardPage() {
  ...
  // 🔥 VERCEL-İ ALDADAN HİSSƏ: useSearchParams əvəzinə təmiz JavaScript
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      } catch (error) { toast.error('Yüklənmə xətası'); }
  ...
  const handleCancel = async (id: string) => {
    ...
    try {
      const res = await apiFetch(`/api/reservations/${id}/cancel`, { method: 'PATCH' });
      if (res.ok) {
    ...
    } catch (err) { toast.error('Bağlantı xətası baş verdi'); }
  };
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      const res = await apiFetch('/api/users/profile', {
        method: 'PUT',
        body: JSON.stringify({ firstName: editForm.firstName, lastName: editForm.lastName, phone: editForm.phone }),
      });
      if (res.ok) {
    ...
    } catch (err) { toast.error("Bağlantı xətası"); }
  ...
  const handleRemoveFavorite = async (e: React.MouseEvent, targetHotelId: number) => {
    ...
    e.stopPropagation();
    try {
      const res = await apiFetch(`/api/favorites/${targetHotelId}`, { method: 'POST' });
      if (res.ok) {
        setSavedHotels((prev) => prev.filter((fav) => {
          const currentId = fav.hotelId || (fav.hotel && fav.hotel.id) || fav.id;
          return currentId !== targetHotelId;
        }));
        toast.success("Siyahıdan çıxarıldı");
      } else { toast.error("Xəta baş verdi, silinmədi."); }
    } catch (err) { toast.error("Bağlantı xətası"); }
  };
          ...
          <aside className="w-full lg:w-80 flex-shrink-0 space-y-6">
            <div className="bg-white rounded-[40px] p-8 border border-slate-100 shadow-xl shadow-slate-200/40 relative overflow-hidden group">
              ...
              <div className="mt-10 space-y-2 relative z-10">
                {NAV_ITEMS.map((item) => (
                  <button key={item.id} onClick={() => setActiveNav(item.id)} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${activeNav === item.id ? 'bg-[#1E3A5F] text-white shadow-xl shadow-blue-900/20' : 'text-slate-400 hover:bg-slate-50'}`}>
              ...
              <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <Tabs defaultValue="upcoming" className="w-full">
                  <TabsList className="bg-slate-100/50 p-1.5 rounded-2xl h-14 mb-8">
                    <TabsTrigger value="upcoming" className="rounded-xl px-8 font-black text-[10px] uppercase tracking-widest">Qarşıdan gələn ({bookings.filter(b => b.status === 'confirmed').length})</TabsTrigger>
                    <TabsTrigger value="past" className="rounded-xl px-8 font-black text-[10px] uppercase tracking-widest">Keçmiş ({bookings.filter(b => b.status === 'completed').length})</TabsTrigger>
                    <TabsTrigger value="cancelled" className="rounded-xl px-8 font-black text-[10px] uppercase tracking-widest">Ləğv edilmiş ({bookings.filter(b => b.status === 'cancelled').length})</TabsTrigger>
                  </TabsList>
                  <TabsContent value="upcoming" className="space-y-6 animate-in fade-in duration-500">
                    {bookings.filter(b => b.status === 'confirmed').length > 0 ? (
                      bookings.filter(b => b.status === 'confirmed').map(b => <BookingCard key={b.id} booking={b} onCancel={handleCancel} />)
                    ) : (
                      <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[40px] border-2 border-dashed border-slate-100">
                        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-200 mb-4"><Calendar size={32} /></div>
                        <p className="text-slate-400 font-bold italic">Aktiv rezervasiyanız yoxdur.</p>
                        <Link href="/search"><Button variant="link" className="text-orange-500 font-black text-xs uppercase tracking-widest mt-2">İndi axtarışa başla</Button></Link>
                      </div>
                    )}
                  </TabsContent>
                  <TabsContent value="past" className="space-y-6 animate-in fade-in duration-500">
                    {bookings.filter(b => b.status === 'completed').length > 0 ? (
                      bookings.filter(b => b.status === 'completed').map(b => <BookingCard key={b.id} booking={b} onCancel={handleCancel} />)
                    ) : (
                      <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[40px] border-2 border-dashed border-slate-100">
                        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-200 mb-4"><Check size={32} /></div>
                  ...
                  <TabsContent value="cancelled" className="space-y-6 animate-in fade-in duration-500">
                    {bookings.filter(b => b.status === 'cancelled').length > 0 ? (
                      bookings.filter(b => b.status === 'cancelled').map(b => <BookingCard key={b.id} booking={b} onCancel={handleCancel} />)
                    ) : (
                      <div className="flex flex-col items-center justify-center py-24 bg-white rounded-[40px] border-2 border-dashed border-slate-100">
                        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center text-slate-200 mb-4"><X size={32} /></div>
              ...
              <div className="bg-white rounded-[40px] p-10 border border-slate-100 shadow-xl shadow-slate-200/40 animate-in fade-in slide-in-from-bottom-4 duration-500">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="space-y-2">
                    ...
                    <input value={editForm.firstName} onChange={(e) => setEditForm({ ...editForm, firstName: e.target.value })} className="w-full bg-slate-50 border-none rounded-2xl h-14 px-6 font-bold text-slate-700 focus:ring-2 focus:ring-orange-500/20 transition-all outline-none" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Soyadınız</label>
                    <input value={editForm.lastName} onChange={(e) => setEditForm({ ...editForm, lastName: e.target.value })} className="w-full bg-slate-50 border-none rounded-2xl h-14 px-6 font-bold text-slate-700 focus:ring-2 focus:ring-orange-500/20 transition-all outline-none" />
                  </div>
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Telefon</label>
                    <input value={editForm.phone} onChange={(e) => setEditForm({ ...editForm, phone: e.target.value })} className="w-full bg-slate-50 border-none rounded-2xl h-14 px-6 font-bold text-slate-700 focus:ring-2 focus:ring-orange-500/20 transition-all outline-none" />
                  </div>
                  <div className="space-y-2">
                ...
                <Button onClick={handleUpdateProfile} disabled={isUpdating} className="mt-12 bg-[#1E3A5F] hover:bg-orange-500 text-white rounded-2xl h-14 px-10 font-black uppercase tracking-widest shadow-xl shadow-blue-900/10 transition-all flex items-center justify-center">
                  {isUpdating ? <span className="flex items-center gap-2"><Loader2 size={18} className="animate-spin" /> Yenilənir...</span> : "Dəyişiklikləri Saxla"}
                </Button>
              </div>
            )}
            {activeNav === 'saved' && (
                      ...
                      return (
                        <div key={fav.id} className="group bg-white rounded-[32px] border border-slate-100 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500">
                          <div className="relative h-48 overflow-hidden">
                            ...
                            <button onClick={(e) => handleRemoveFavorite(e, realHotelId)} className="absolute top-4 right-4 w-10 h-10 bg-white/80 backdrop-blur-md rounded-full flex items-center justify-center text-red-500 shadow-lg hover:bg-red-500 hover:text-white transition-all z-10"><HeartOff size={18} /></button>
                          </div>
                          <div className="p-6">
                            <h3 className="font-black text-slate-900 text-lg line-clamp-1 mb-1">{hotelName}</h3>
                            <p className="text-xs font-bold text-slate-400 flex items-center gap-1 mb-4 uppercase tracking-wider"><MapPin size={12} className="text-orange-500" /> {city}</p>
                            <div className="flex justify-between items-center pt-4 border-t border-slate-50">
                              ...
                              <Link href={`/hotel/${realHotelId}`}><Button size="sm" className="bg-[#1E3A5F] hover:bg-orange-500 rounded-xl px-6 h-10 font-black text-[10px] uppercase tracking-widest">Bax</Button></Link>
                            </div>
                          </div>
                        </div>
                      );
                        ...
                        <div className="flex justify-between items-start">
                          <div className="space-y-3">
                            <div className="flex items-center gap-3">
                              <div className="w-10 h-10 bg-orange-50 rounded-xl flex items-center justify-center text-orange-500"><Building size={20} /></div>
                              <div>
                                <h4 className="font-black text-slate-900 text-lg leading-none">{rev.hotelName || "Otel Adı"}</h4>
                                <div className="flex gap-0.5 mt-1">
                                  {Array.from({ length: 5 }).map((_, i) => <Star key={i} size={10} className={i < rev.rating ? "fill-orange-400 text-orange-400" : "text-slate-100"} />)}
                                </div>
                              </div>
                            </div>
                            <p className="text-slate-600 text-sm font-medium leading-relaxed italic border-l-4 border-slate-100 pl-4 py-1">"{rev.comment}"</p>
                            <p className="text-[10px] font-black text-slate-300 uppercase tracking-[2px]">{new Date(rev.createdAt).toLocaleDateString('az-AZ')}</p>
                          </div>
                          <button onClick={() => handleDeleteReview(rev.id)} className="p-3 text-slate-300 hover:text-red-500 hover:bg-red-50 rounded-2xl transition-all"><Trash2 size={20} /></button>
                        </div>
                      </div>
                    ))}
                  </div>
